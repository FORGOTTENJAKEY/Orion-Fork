--[[
    LoadingScreen — standalone, no OrionLib/OrionThemes dependency

    Usage:
        local LoadingScreen = require(game.ReplicatedStorage.LoadingScreen)
        ...
        Screen:SetProgress(0,   "Initialising modules")
        task.wait(1)
        Screen:SetProgress(50,  "Loading assets")
        task.wait(1)
        Screen:SetProgress(100, "Done!")
        Screen:Destroy()

    SetProgress(progress: number?, extra: string?)
        progress  0–100, nil keeps the current value
        extra     status line beneath the bar, nil clears it

    SetTitle(title: string)
    SetSubtitle(subtitle: string)
    Destroy(tweenOut: boolean?)   – true (default) plays the fade-out tween first
]]
...
--[[
    ...
    progress  0–100   nil = keep current value
    extra     string  status label shown below the bar
                      nil = clear the label
                      false = leave label unchanged
]]
...
--[[
    Destroy(tweenOut: boolean?)
    tweenOut = true (default) plays fade-out before removing the gui
    tweenOut = false destroys immediately
]]
function LoadingScreen:Destroy(tweenOut)
	...
	-- slide bar to 100% if not already there
	if self._progress < 100 then
		tween(self._barFill, FAST, { Size = UDim2.new(1, 0, 1, 0) })
		self._pct.Text = "100%"
	end
